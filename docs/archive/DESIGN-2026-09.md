...
**Archived design, 2026-09-06.** This is the reasoning behind Anabtawi OS. `SPEC.md` and `AGENTS.md` are normative; where this document conflicts with them (cadence, notification channel, cost, guardrail values, column ids, build order), they are right and this is history. The six audits in `docs/audit/` list every correction.
...
**What it costs.** About USD 350 (CAD 485) a month in new tools for six months, falling to about USD 250 (CAD 350) (DataDoe USD 97, Keepa €49, Link My Books USD 41, Helium 10 USD 99 for six months only, Getida on success fee). Subscriptions already held cover the models. Budget USD 50 a month of API credits as an overflow lane. Second brand: one afternoon, one new API key.
...
Moved to `docs/audit/` context: B (git spine, monday surface, Mac mini hands) scored 87 of 100 against Rami's weights; the orchestrator platform 68; hosted vendor agents 51; monday-native 48. The reasons are in the research reports.
...
│  SKU Profiles · Tasks for me · Scorecard          monday push: P0, card   │
└───────────────▲───────────────────────────────────────────────▲─────────────┘
...
| Notifications | monday push only | Slack, monday push, email, Pushover | monday is where the tap happens; one channel, and a P0 while monday is unreachable is accepted as lost |
...
5. **Stable column ids.** The MCP cannot set column ids, so `bin/build-monday.py` creates columns with raw GraphQL `create_column(id:)`; ids are `[a-z_]{1,20}`, never reused, never renamed. Board ids are cached in `ops/monday-ids.json`.
...
| Run Health | when `status` changes to Failed or Stale → notify Rami | the only failure channel |
...
- **Nightly integrity check** (fifteen assertions, from orphan detection to `fulfillable + inbound + reserved + unfulfillable + researching == total`, price inside band, margin above floor, meltable inbound only 16 Oct – 14 Apr, nothing inbound with under 105 days of shelf life at receipt (reported Amazon rule), ad spend under the daily cap, revenue reconciles to DataDoe within 2%). Writes `state/integrity.md` and opens a request on failure.
...
7. Commit `<dept>: <date> <slot>` and push; the wrapper pings Healthchecks "done", writes Run Health, and on non-zero exit or timeout writes the failure state file itself so Run Health turns Failed.
...
6. **Present**: the CEO run gates, scores and ranks; `bin/project-monday.py` creates or updates the Decisions item with the diff; monday push and monday push carry the card.
...
- **Morning (optional, 30 seconds):** a monday push only if a P0 fired overnight or a department failed. Otherwise nothing.
...
| Vault, monitoring, GitHub | ~USD 10 | ~USD 10 |
